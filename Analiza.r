...
bledy.uczacy <-errorevol(drzewo.bagging, df.bagging.uczacy)
bledy.walidacyjny <-errorevol(drzewo.bagging, df.bagging.walidacyjny)

plot.errorevol(x = bledy.walidacyjny,
               y = bledy.uczacy)

#Zmiana argumentu cp dla pojedynczych drzew
drzewo.baggingv2 <- bagging(
  formula = status~.,
  data = proba_uczaca_model,
  mfinal = 20,
...
bledy.uczacy <-errorevol(drzewo.baggingv2, df.bagging.uczacy)
bledy.walidacyjny <-errorevol(drzewo.baggingv2, df.bagging.walidacyjny)
...
View(proba_testowa_merged)




# Przeanalizować rozkłady głosów
head(drzewo.bagging.final$votes)
...
barplot(table(proba_uczaca_model$status, drzewo.bagging.final$votes[, 2]),
        legend.text = T,
        args.legend = list(x = "top",
                           title = "Rzeczywista klasa"),
        xlab = "Liczba głosów na TAK")

table(proba_uczaca_model$status, drzewo.bagging_final$votes[, 2])
...
head(proba_uczaca_merged_nodel)
...
init_screenwidth,screenwidth,screenheight,other_software_info,expirymonth,expiryyear,expiry_date,init_expirymonth,init_expiryyear,compare_to_expiry,expiry_date, init_mccname,init_listtype,expired_card,recurringaction))
...
set.seed(11)
x <- runif(200, 0, 100)
y <- -0.04*x*(x-130) + rnorm(200, mean = 0, sd = 20)

plot(x, y)

# Zbudować model liniowy, drzewo regresyjne oraz model MARS
m.lin <- lm(y ~ x)
m.cart <- rpart(y ~ x)
m.mars <- earth(y ~ x)

# Przedstawić predykcje z trzech modeli na wykresie
plot(x, y)
points(x, predict(m.lin), pch = 16, col = 2)
points(x, predict(m.cart), pch = 16, col = 3)
points(x, predict(m.mars), pch = 16, col = 4)

legend(x = "bottomright",
       legend = c("LM", "CART", "MARS"),
       col = 2:4,
       pch = 16)

# Zinterpretować parametry strukturalne w modelu MARS
summary(m.mars)
 # Wraz ze wzrostem x o jedną jednostkę, y zwiększa się średnio o 3.36 jednostek, pod warunkiem, że x < 45.09
 # Dla x między 45.09 a 73.40, wartość zmiennej y nie zależy od wartości zmiennej x (tu: y jest średnio na poziomie równym wyrazowi wolnemu)
 # Wraz ze wzrostem x o jedną jednostkę, y zmniejsza się średnio o 1.44 jednostek, pod warunkiem, że x > 73.40


#### Model dla ceny mieszkania w Trójmieście #### 
# Załadować dane m3m.v2 (dane dot. ofert mieszkaniowych z rynku wtórnego w Trójmieście)
load("m3m.v2.RData")

m3m.v2$cena_m2 <- m3m.v2$cena_m2/1000  # cena w tys. zł

sum(is.na(m3m.v2))  # nie ma braków
str(m3m.v2)

# Zbudować model addytywny MARS ze zmienną objaśniającą 'cena_m2' (Cena za m2 mieszkania)
mars1 <- earth(cena_m2 ~ .,
               data = m3m.v2)
summary(mars1)
